...
# 调整词法输出
def change_output_lexer(tag,value):
    new_dict = {v : k for k, v in Key.items()}
    for i in range(len(tag)):
        tag_ = tag[i]
        value_ = value[i]
        dict_tag = new_dict[tag_]
        if value_ == 'None':
            value_ = ''
#         print("<%s,%s>" % (value_,dict_tag.value))
        with open('lexer_res.txt',"a+") as f:
            f.writelines("<%s,%s>" % (value_,dict_tag.value))
            f.writelines("\n")
